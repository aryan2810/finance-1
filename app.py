...
def index():
    ...
    for sale in sales:
        stocks[sale['symbol']]['total_shares'] = stocks[sale['symbol']
                                                        ]['total_shares'] - sale['total_shares']
...
def register():
    ...
    if request.method == "POST":
        ...
        if (regex.search(request.form.get("password")) == None):
            ...
            db.execute("INSERT INTO users (username, hash) VALUES (:username, :hashed_password)", username=request.form.get(
                "username"), hashed_password=generate_password_hash(request.form.get("password")))
            return render_template("login.html")
...
def sell():
    ...
    if request.method == "POST":
        ...
        user_shares = int(purchases[0]['total_shares']
                          ) - int(sales[0]['total_shares'])
